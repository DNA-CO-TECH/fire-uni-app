	...
	<view class="container">
		...
		<view class="search-bar" :style="`padding-top:${navHeight + 20}px;`">
			...
			<view class="search-input">
				<input style="font-size: 14px;" type="text" placeholder="通过手机号搜索" @confirm="handleSearch"
					@input="onKeyInput" />
				...
				<view class="item-list" v-for="(item, itemIndex) in orderList" :key="itemIndex">
					...
					<view class="order-list fcc-start gap-12">
						<view v-for="(order, index) in item.orders" :key="index" :index="index" class="order-item">
							<liu-swipe-action :index="index" @clickItem="(btn) => { clickItem(item, order, btn) }"
								:btnList="btnList">
								<view class="slide-left w-full frc-start gap-8">
									<!-- 头像 -->
									<view>
										<image class="avatar" src="https://dnamini-1316443200.cos.ap-shanghai.myqcloud.com/yanstatic/manage/fire.svg" />
									</view>
											...
											{{ order.desk.some(num => num > 3) ? `吧台${order.desk.filter(d => d <= 3).length >
												0 ? ',' + order.desk.filter(d => d <= 3) + '号桌' : ''}` : `${order.desk}号桌` }} ，客人：{{
													...
													<view class="phone-info frc-start">
														...
														<button class="copy"
															@click.stop="copyPhone(order.phone)">复制</button>
													</view>
										</view>
									</view>
							</liu-swipe-action>
