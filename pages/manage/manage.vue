		...
		<view class="nav-wrap" :style="`height:${navHeight}px;`">
			...
			<view class="capsule-box"
				...
				<image class="nav-logo"
					src="https://dnamini-1316443200.cos.ap-shanghai.myqcloud.com/yanstatic/manage/switch.svg"
					@click="handleSwitch"></image>
		...
		<data-list :page="page" :size="size" emptyText="~ 我是有底线的 ~" @load="handleLoad" @refresh="handleRefresh"
			...
			<view class="list-container">
				<view class="item-list" v-for="(item, itemIndex) in orderList" :key="itemIndex">
					<view class="item-content frc-start gap-40">
						<!-- 日期 -->
						<view class="item-date">{{ new Date(item.dateInfo).getMonth() + 1 }}月{{ new
							Date(item.dateInfo).getDate()}}日
						</view>
						<!-- 时间段 -->
						<view class="item-time">{{ item.timeInfo === 0 ? '17:30~18:30' : '19:00 ~20:00' }}</view>
						<!-- 共x位客人 -->
						<view class="item-guests">
							共{{ item.orders.map(o => o.people).reduce((accumulator, currentValue) => accumulator +
								currentValue, 0) }}位客人
						</view>
					</view>
					<view class="order-list fcc-start gap-24">
						<view v-for="(order, index) in item.orders" :key="index" :index="index" class="order-item"
							@click.stop="handleEdit(item,order)">
							<liu-swipe-action :index="index" @clickItem="(btn) => { clickItem(item, order, btn) }" :btnList="btnList">
								<view class="slide-left w-full frc-start gap-20">
									<!-- 头像 -->
									<view>
										<image class="avatar"
									...
									<view class="order-info">
										...
										<view class="desk-info frc-between" style="font-size: 14px;">
											{{ order.desk.some(num => num > 3) ? `吧台${order.desk.filter(d => d <= 3).length >
												0 ? ',' + order.desk.filter(d => d <= 3) + '号桌' : ''}` : `${order.desk}号桌` }} ，客人：{{
													order.people }}位
										</view>
										<!-- 手机号 + 复制按钮 -->
	...
	export default {
		...
		methods: {
			...
			async handleEdit(item, order) {
				...
				console.log("click-orderDetail", orderDetail);
			...
			addOrder() {
				...
				uni.navigateTo({
					url: `/pages/editOrder/editOrder?isAdd=true`
				})
			},
			// 删除预订
