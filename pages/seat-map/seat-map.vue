<template>
	<view class="root flex flex-col">
		<view class="nav-wrap" :style="{
			...
			<view class="capsule-box"
				...
				<image class="nav-logo"
					:src="`${isEdit ? 'https://dnamini-1316443200.cos.ap-shanghai.myqcloud.com/yanstatic/manage/left-arrow.svg' : 'https://dnamini-1316443200.cos.ap-shanghai.myqcloud.com/yanstatic/book/left-arrow-orange.svg'}`"
		...
		<view class="book-container fcc-start" :style="{
	    'background-color': isEdit ? '#FFFFFF' : '',
	    'margin-top': `${navHeight}px`,
	    'min-height': `calc(100% - ${navHeight}px)`
	  }">
			<view class="container z-1 fcc-center flex-1">
				<view class="bar-tip mt-20" style="text-align:left" v-show="!isEdit"><text>点击下方座位选座</text></view>
					...
					<view class="left-wrap">
						<view class="left">
							...
							<view class="table1-wrap">
								<view class="flex seat-row">
									<view v-for="seatIndex in [1, 2]" :key="seatIndex" @click="toggleBookSeats(0, seatIndex)" :class="{
										...
										<image :style="`width:14px;height:14px;${isEdit?'filter: invert(100%)':''}`"
											v-show="myBookSeatsIds.includes(seatIndex)"
								...
								'table': true,
								'booked': tableStatus[0] === 0,
								'not-booked-desk': tableStatus[0] !== 0
							}">
							...
							<view class="table2-wrap">
								...
								'table2': true,
								'booked': tableStatus[1] === 0,
								'not-booked-desk': tableStatus[1] !== 0
							}">
									{{ tableStatus[1] === 0 ? '不可预订' : "点击预订" }}
								</view>
								<view class="fcc-center gap-24">
							...
							<view class="table3-wrap">
								...
								'table': true,
								'booked': tableStatus[2] === 0,
								'not-booked-desk': tableStatus[2] !== 0
							}">
					...
					<view class="right">
						...
						<view class="bar-wrap">
							<view class="fcc-between" :style="{ padding: '20px 0px' }">
								<view @click="toggleBookSeats(tableIndex)" v-for="tableIndex in [3, 4, 5, 6, 7, 8, 9, 10]"
									:key="tableIndex" :class="{
										'seat': true,
										'seat-left': true,
									...
									<image :style="`width:14px;height:14px;${isEdit?'filter: invert(100%)':''}`"
										v-show="myBookTableIds.includes(tableIndex)"
										src="https://dnamini-1316443200.cos.ap-shanghai.myqcloud.com/yanstatic/book/right.svg" />
								</view>
							</view>
							<view class="bar-back fcs-between relative">
								<view class="bar">
									<view style="width: 22px;border:none" :class="{
										...
										'not-booked-desk': leftBarSeats !== 0 && tableStatus.some(table => table === 1)
									}">
										<text>开放式吧台</text>
				...
				<view class="frc-center gap-40" style="width: calc(100% - 24px); margin:'0px 12px';height: 172rpx"
					v-if="isEdit">
					<button class="flex-1 next-step" style="color:#000000;background-color: #DFDFDF" @click="clear">清空选择</button>
				...
				<view class="w-full" v-else>
					...
					<view class="frc-between submit-wrap">
						<view class="people-info frc-center">
							<view class="icon-wrap frc-center">
								<image src="https://dnamini-1316443200.cos.ap-shanghai.myqcloud.com/yanstatic/book/right.svg"
									style="width: 12px;height: 12px;" />
							</view>
							<text style="color:gray">已选择 {{ peopleCount }} 位就餐</text>
						</view>
						<button class="next-step" @click="confirm" style="height:80rpx">立即预订</button>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			async initDayBookInfo() {
				...
				console.log("reqOneDaySeatsLeft-res", res);
				this.seatsLeft = res
			},
			...
			getTableBookArray(val) {
				console.log("getTableBookArray-val", val);
				...
				if (this.isEdit) {
					...
					this.myBookSeatsIds = orderDetail.tempSeats ? orderDetail.tempSeats : this.initSeatsIds((orderDetail.desk ||
					[]))
...
<style lang="scss" scoped>
	...
	.book-container {
		...
		.bar-back {
			background-image: url('https://dnamini-1316443200.cos.ap-shanghai.myqcloud.com/yanstatic/book/bar.svg');
		...
		.light {
			...
			right: 0px;
			z-index: -1;
			background-image: url('https://dnamini-1316443200.cos.ap-shanghai.myqcloud.com/yanstatic/book/light.png');
