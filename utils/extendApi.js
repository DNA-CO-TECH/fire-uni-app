...
/**
 * @description 消息提示框
 * @param { Object } options 参数和 uni.showToast 参数保持一致
 */
const toast = ({
	title = '数据加载中...',
	icon = 'none',
	...
	// 如果用户传入了对象作为参数,在形参位置通过解构的方式获取用户传入的参数，同时设置默认值
	uni.showToast({
		title,
		icon,
		duration,
...
/**
 * @description 模态对话框
 * @param { Object } options 参数和 uni.showModal 参数保持一致
 */
const modal = (options = {}) => {
	// 在调用 modal 方法时，可以传递参数，也可以不传递参数
	// 如果不传递参数，默认值就是空对象
	// 如果传递参数，参数需要时一个对象，对象中的属性需要和 uni.showModal 参数保持一致
	...
	return new Promise((resolve) => {
		...
		uni.showModal({
			// 将合并以后的参数通过展开运算符赋值给 uni.showModal 对象
			...opts,
			complete({
				confirm,
				cancel
...
// 可以将 toast 方法挂载到 uni 全局对象身上
// 如果想挂载到 uni 全局对象上这种写法生效，需要让当前文件执行一次
uni.toast = toast
uni.modal = modal
